...
def OIII_OII_ratio(master_table):
    ...
    #master_table = Table.read(table_file, format='ascii.fixed_width_two_line')
    OIII5007 = -1
    OIII5007_err = -1
    ...
    for i, val in enumerate(master_table['Line Name']):
        ...
        if val == '[OII]3725':
            OIIind.append(i)
        if val == '[OIII]5007':
            OIII5007 = master_table[i][2]
            OIII5007_err = master_table[i][3]
    ...
    #flux measurment is in column 2 of the table and flux err is in column 3
    OII3727 = master_table[row_index][2]
    OII3727_err = master_table[row_index][3]

    ratio_err = np.sqrt(OII3727_err**2 + OIII5007_err**2)

    ratio = OIII5007/OII3727

    return ratio, ratio_err

def line_ratio(master_table, line1_, line2_):
    ...
    for i, val in enumerate(master_table['Line Name']):
        ...
        if val == line1_:
            line1 = master_table[i][2]
            line1_err = master_table[i][3]

        if val == line2_:
            line2 = master_table[i][2]
            line2_err = master_table[i][3]
...
def OxygenIII_ratios(master_table):
    #master_table = Table.read(table_file, format='ascii.fixed_width_two_line')
    ...
    for i, val in enumerate(master_table['Line Name']):
        ...
        if val == '[OIII]4959':
            OIII4959 = master_table[i][2]
            OIII4959_err = master_table[i][3]

        if val == '[OIII]4363':
            OIII4363 = master_table[i][2]
            OIII4363_err = master_table[i][3]
    ...
    ratio = (OIII5007 + OIII4959)/OIII4363
...
def galaxy_temperature(ratio):
    ...
    ratio_line = ratio * np.ones(len(Temp))
...
finaltable = [x for x in glob.glob('*finaltable.fits')]
...
for i in finaltable:
    ...
    ratio_OIII_hbeta_err.append(OIII_Hbeta_err)
    ...
    OIII_5007_3727.append(OIII_OII)
    OIII_5007_3727_err.append(OIII_OII_err)
...
ratio_table['OIII5007/OII3727 err'] = np.array(OIII_5007_3727_err)
